...
function export_version() {
    export FILES_API_VERSION=$(tr -d "\n\r" < ../VERSION | xargs)
    echo "FILES_API_VERSION: $FILES_API_VERSION"
}
...
if [[ "$1" != "start" && "$1" != "stop" && "$1" != "down" ]]; then
    ...
    if [ "$1" = "start" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p files-api up -d
    elif [ "$1" = "stop" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p files-api stop
    elif [ "$1" = "down" ]; then
        docker-compose -f docker-compose.yml -f docker-compose.deployment.yml -p files-api down
    fi
fi
